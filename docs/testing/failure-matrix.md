...
| PostgreSQL primary failover           | stop primary, promote physical standby, switch Toxiproxy upstream      | RPO=0, прежний result, bounded recovery write path            | `PostgresCriticalDependencyUnavailable` | Ledger Platform  | `pnpm resilience:postgres-failover`                    |
| CPU/memory/FD/event-loop pressure     | bounded Node workers, nofile=96, Docker pause/unpause обеих replicas   | bounded degradation и recovery без потери durable effect      | `RuntimeResourceSaturation`             | Platform         | `pnpm resilience:resource-pressure`                    |
