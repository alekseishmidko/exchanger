...
имеет права использовать in-memory state. Lease/fencing, durable projections и
fault-proxy topology относятся к следующим подразделам 17A и пока не закрыты.
...
`EVENT_LOG_PORT`, `PROJECTION_STORE_PORT`, `AUDIT_LOG_PORT`. Интерфейсы выражают
use cases (`reserve`, `append`, `apply`), а не методы PostgreSQL или broker SDK.
